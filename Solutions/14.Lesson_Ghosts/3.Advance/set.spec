...
definition ACTIVATE_ADDRESS_SET(uint256 i, address a) returns bool = (mirrorArrayLenInit == getLen()) && addressSetActive && (lengthOfValidInternalIndexesMap) == 0 && (lengthOfValidInternalIndexes == 0);
...
hook Sload bytes32 value _list .(offset 0)[INDEX uint256 index] STORAGE {
    ...
    bool firstAccess = firstAccessArray ;//&& firstAccessMap;
...
hook Sstore _list .(offset 0)[INDEX uint256 index] bytes32 newValue (bytes32 oldValue) STORAGE {
    ...
    // uint256 shortcutIndexMap = reverseValidInternalIndexesMap[oldValue];
    // bool firstAccessMap = ((shortcutIndexMap >= lengthOfValidInternalIndexesMap) || validInternalIndexesMap[shortcutIndexMap] != oldValue);
    //reverseValidInternalIndexesMap[oldValue] = firstAccessMap?lengthOfValidInternalIndexesMap:reverseValidInternalIndexesMap[oldValue];
    //validInternalIndexesMap[lengthOfValidInternalIndexesMap] = oldValue;
    //lengthOfValidInternalIndexesMap = lengthOfValidInternalIndexesMap + to_uint256(firstAccessMap?1:0);
    //require mirrorMapInit[oldValue] == (firstAccessMap?index:mirrorMapInit[oldValue]);
...
hook Sstore _list .(offset 32)[KEY bytes32 value] uint256 newIndex (uint256 oldIndex) STORAGE {
    // uint256 shortcutIndex = reverseValidInternalIndexes[oldIndex];
    // bool firstAccessArray = ((shortcutIndex >= lengthOfValidInternalIndexes) || validInternalIndexes[shortcutIndex] != oldIndex);
    // reverseValidInternalIndexes[oldIndex] = firstAccessArray?lengthOfValidInternalIndexes:reverseValidInternalIndexes[oldIndex];
    // validInternalIndexes[lengthOfValidInternalIndexes] = oldIndex;
    // lengthOfValidInternalIndexes = lengthOfValidInternalIndexes + to_uint256(firstAccessArray?1:0);
    // require mirrorArrayInit[oldIndex] == (firstAccessArray?value:mirrorArrayInit[oldIndex]);
...
invariant addressSetInvariant(uint256 i, address a)
    ...
    {
        preserved{
            require ACTIVATE_ADDRESS_SET(i, a);
        }
    }
